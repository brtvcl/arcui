				...
				return selected.map((s) => {
					return h("span", { class: "arc-combobox-tag" }, [
						...
						h(
							"span",
							{
								class: "arc-combobox-tag-button",
								dangerouslySetInnerHTML: { __html: xMarkSvg },
		...
		function createComponent() {
			...
			function selectedRemoveClickHandler(removed: Option) {
				...
				setSelected((prevSelected) =>
					prevSelected.filter((s) => JSON.stringify(s.value) != JSON.stringify(removed.value))
				);
			...
			return h(
				...
				[
					...
					dropdownVisible
						? h("div", { class: "arc-combobox-dropdown", style: `width: ${state.width}` }, [
								h(
									"ul",
									null,
									options.length
